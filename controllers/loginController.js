...
const login = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      console.log(
        "Role:",
        user.Role ? user.Role.nama_role : "Role tidak ditemukan"
      );
      console.log("Panjang password di DB:", user.password.length);
      console.log(
        "Apakah password seperti hash bcrypt?",
    ...
    if (user.password.startsWith("$2b$") || user.password.startsWith("$2a$")) {
      console.log(
        "Password di database sudah di-hash, menggunakan bcrypt.compare"
      );
      isMatch = await bcrypt.compare(password, user.password);
    } else {
      console.log(
        "Password di database masih plain text, membandingkan langsung"
      );
      isMatch = password === user.password;

      if (isMatch) {
        console.log(
          "Login berhasil, akan meng-hash password untuk keamanan..."
        );
